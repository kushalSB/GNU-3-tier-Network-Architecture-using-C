...
char *get_all_data_from_table(MYSQL *conn, const char *table_name)
{
    ...
    int rows_found = 0; // Flag to track if any rows are found
    while ((row = mysql_fetch_row(res)) != NULL)
    {
        rows_found = 1; // Set the flag as rows are found
    ...
    // If no rows were found, return a special string indicating no data
    if (!rows_found)
    {
        strcpy(result, "No Data Found"); // Free the previously allocated memor
    }
...
char *get_one_data_from_table(MYSQL *conn, const char *table_name, int id)
{
    ...
    if (!rows_found)
    {
        strcpy(result, "Database: No Data Found"); // Free the previously allocated memor
    }
